...
import { getRespaldos, getRespaldosUsers, getRespaldosExcel } from "../api/calidadAPI";
...
function RespaldosView() {
    const authState = useSelector((state) => state.auth);
    const { user_id, area, permisos, token } = authState;
    ...
    const respaldoTable = () => (
        <Box sx={{ overflowX: "auto", width: "95%", my: 4 }}>
            <Table sx={{ minWidth: 650 }} aria-label="reparaciones-table" stickyHeader size="small">
                <TableHead>
                    <TableRow>
                        {["FECHA", "ORDEN", "TECNICO", "PELO", "PUERTO", "CTO", "VECINO"].map((header) => (
                            <TableCell
                                key={header}
                                align="center"
                                sx={{
                    ...
                    {data && data.length > 0 ? (
                        data.map((row, index) => (
                            ...
                            >
                                ...
                                <TableCell align="center" width={"15%"}>{row.img_pelo ? <Chip color="success" /> : <Chip color="error" />}</TableCell>
                                <TableCell align="center" width={"15%"}>{row.img_puerto ? <Chip color="success" /> : <Chip color="error" />}</TableCell>
                                <TableCell align="center" width={"15%"}>{row.img_cto ? <Chip color="success" /> : <Chip color="error" />}</TableCell>
                                <TableCell align="center" width={"15%"}>{row.img_vecino ? <Chip color="success" /> : <Chip color="error" />}</TableCell>
                            </TableRow>
                        ))
                    ) : (
                        <TableRow>
                ...
                >
                    <Box sx={{ display: "flex", gap: 3, flexWrap: "wrap", justifyContent: "center", alignItems: "center", mt: 1 }}>
                        ...
                        <FormControl variant="standard" sx={{ minWidth: 250 }} size="small">
                            <InputLabel id="tecnico-label">Tecnico</InputLabel>
                            <Select
                            ...
                            >
                                ...
                                {users.sort((a, b) => a.nombre.localeCompare(b.nombre)).map((tipo) => (
                                    <MenuItem key={tipo.userID} value={tipo.userID}>
                                        {tipo.nombre}
                                    </MenuItem>
                                ))}
                            </Select>
    ...
    const fetchData = async () => {
        ...
        setIsSubmitting(true);
        try {
            const response = await getRespaldos(page, filterForm);
            const usersResponse = await getRespaldosUsers(token);
