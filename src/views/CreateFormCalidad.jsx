...
function FormCalidad() {
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-start', minHeight: '100vh', py: 6, background: palette.bgGradient, position: 'relative', '::before': { content: '""', position: 'absolute', inset: 0, background: 'linear-gradient(160deg, rgba(255,255,255,0.06), rgba(255,255,255,0) 60%)', pointerEvents: 'none' } }}>
      {open && (
      ...
      <Card sx={{ width: '55%', maxWidth: 880, borderRadius: 4, overflow: 'hidden', background: palette.cardBg, border: `1px solid ${palette.borderSubtle}`, backdropFilter: 'blur(6px)', boxShadow: '0 18px 42px -16px rgba(0,0,0,0.55), 0 8px 16px -6px rgba(0,0,0,0.35)' }}>
        <CardHeader title={<Typography fontWeight={700} letterSpacing={.5}>FORMULARIO CALIDAD</Typography>} avatar={<FeedIcon />} sx={{ background: `linear-gradient(140deg, ${palette.primary} 0%, ${palette.primaryDark} 100%)`, color: 'white' }} />
