...
function AtencionTotem() {
  ...
  const fetchData = async () => { 
    ...
    try { 
      ...
      setFila(res.filter(r => r.Estacion_ID === selectedEstacion.EstacionID));
  ...
  useEffect(() => {
    setEspera(fila.filter(f => f.Estado === 'En espera'));
    (async () => { 
      if (!selectedEstacion) return; 
      try { 
        console.log("ESTACION SELECCIONADA : ", selectedEstacion.EstacionID);
        console.log("MODULO SELECCIONADO : ", selectedEstacion.Modulo);

        const res = await enAtencion(selectedEstacion.EstacionID, selectedEstacion.Modulo); 
        console.log("Atencion Response: ", res);
        setAtencion(res[0]); 
      } catch (e) { 
        console.log(e); 
      } 
    })();
  }, [fila, selectedEstacion]);
  ...
  const reversasTable = (selectedEstacion === 300 && atencion && data.length > 0) ? (
      ...
      <Box sx={{ p: 2, ...glass }}>
        ...
        <TableContainer sx={{ maxHeight: 380 }}>
          <Table stickyHeader size="small">
            ...
            <TableBody>
              {data.map(row => {
                const f = formattedData.find(i => i.id === row.id) || {}; return (
                  <TableRow key={row.id}>
