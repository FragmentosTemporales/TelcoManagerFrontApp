...
import {
    getProyectosFiltradosLinkVNO
} from "../api/onnetAPI";
import { MainLayout } from "./Layout";
import { palette } from "../theme/palette";
import ModuleHeader from "../components/ModuleHeader";

function ProyectosLinkVNOView() {
    ...
    const fetchData = async () => {
        ...
        try {
            const response = await getProyectosFiltradosLinkVNO(toFilter, page);
    ...
    const handleClear = async (e) => {
        e.preventDefault();
        try {
            setToFilter({ proyecto: "" });
            setPage(1); // Reset to the first page
            await getProyectosFiltradosLinkVNO(
                { proyecto: "" },
                1
            ).then((res) => {
                setPages(res.pages);
                ...
                {data && data.length > 0 ? (
                    data.map((row, index) => (
                        <TableRow
                            key={index}
                            component={Link}
                            to={`/modulo:proyecto-filtrado-vno/${row.nombre}`}
              sx={{
                textDecoration: "none",
                cursor: "pointer",
                        ...
                        >
                            <TableCell align="center" sx={{ fontSize: "12px" }}>
                                <Typography variant="secondary">
                                    {row.nombre ?? "Sin Información"}
                                </Typography>
                            </TableCell>

                            <TableCell align="center" sx={{ fontSize: "12px" }}>
                                <Typography variant="secondary">
                                    {row.Comuna ?? "Sin Información"}
                                </Typography>
                            </TableCell>

                            <TableCell align="center" sx={{ fontSize: "12px" }}>
                                <Typography variant="secondary">
                                    {row['Estado Proyecto - Identificador'] ?? "Sin Información"}
                                </Typography>
                            </TableCell>

                            <TableCell align="center" sx={{ fontSize: "12px" }}>
                                <Typography variant="secondary">
                                    {row['Estado Orden_A'] ?? "Sin Información"}
                                </Typography>
                            </TableCell>

                            <TableCell align="center" sx={{ fontSize: "12px" }}>
                                <Typography variant="secondary">
                                    {row['Estado Orden_B'] ?? "Sin Información"}
                                </Typography>
                            </TableCell>
                        </TableRow>
                    ))
...
export default ProyectosLinkVNOView;
