...
function BodegaQuintaViewer() {
  ...
  const estilo = { fontSize: "1rem", paddingTop: "5px", paddingBottom: "5px" };
  ...
  const fetchData = async () => {
    try {
      ...
      const filteredDataBodega = response.filter((item) => item.Estacion_ID === 401);
      const filteredDataLogistica = response.filter((item) => item.Estacion_ID === 301);
  ...
  const set_interval = () => (esperaBodega.length > 0 && esperaLogistica.length > 0 ? 4000 : 10000);

  useEffect(() => {
    const intervalId = setInterval(fetchData, set_interval());
    return () => clearInterval(intervalId);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  useEffect(() => {
    const newAtencion = dataBodega.filter((item) => item.Estado === "En atención");
    verificarAtencion(newAtencion, 1);
    setAtencionBodega(newAtencion);
    setEsperaBodega(dataBodega.filter((item) => item.Estado === "En espera"));
  }, [dataBodega]);

  useEffect(() => {
    const newAtencion = dataLogistica.filter((item) => item.Estado === "En atención");
    verificarAtencion(newAtencion, 2);
    setAtencionLogistica(newAtencion);
    setEsperaLogistica(dataLogistica.filter((item) => item.Estado === "En espera"));
  }, [dataLogistica]);
