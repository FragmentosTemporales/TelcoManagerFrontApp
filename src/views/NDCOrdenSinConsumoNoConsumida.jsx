...
import { fetchDetalleOrdenSinConsumoNoConsumida } from "../api/ndcAPI";
...
function NDCOrdenSinConsumoNoConsumidaView() {
    ...
    const fetchData = async () => {
        ...
        try {
            const response = await fetchDetalleOrdenSinConsumoNoConsumida(orden);
            setData(response || []);
            setDataDetalle(response.detalles_orden || []);
                                    ...
                                    <Table stickyHeader>
                                        <TableHead>
                                            <TableRow>
                                                ...
                                                <TableCell size="small" align="center" sx={{ fontWeight: "bold", background: palette.primary, color: "#fff" }}>Tipo</TableCell>
                                                <TableCell size="small" align="center" sx={{ fontWeight: "bold", background: palette.primary, color: "#fff" }}>Fecha</TableCell>
                                            </TableRow>
                                        </TableHead>
                                        <TableBody>
                                            {dataDetalle.length > 0 ? dataDetalle.map((item, index) => (
                                                <TableRow key={index}>
                                                    ...
                                                    <TableCell size="small" align="center">{item.tipo ? item.tipo : "-"}</TableCell>
...
export default NDCOrdenSinConsumoNoConsumidaView;
