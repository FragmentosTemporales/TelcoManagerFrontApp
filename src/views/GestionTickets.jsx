...
function GestorTicketera() {
  ...
  const chipStyles = (estado) => {
    const base = estadoColors[estado] || palette.primary;
    return {
      fontSize: "11px",
      fontWeight: "bold",
      color: "#fff",
      px: 0.75,
      letterSpacing: ".5px",
      background: `linear-gradient(135deg, ${base} 0%, ${base}cc 85%)`,
      boxShadow: "0 2px 6px -1px rgba(0,0,0,0.35)",
  ...
  const filterTickets = () => (
    <Box
      sx={{
        ...
        boxShadow:
          "0 6px 16px -4px rgba(0,0,0,0.15), 0 2px 6px -1px rgba(0,0,0,0.08)",
        transition: "box-shadow .35s, transform .35s",
        "&:hover": {
          boxShadow: "0 10px 26px -6px rgba(0,0,0,0.30)",
        ...
        "&:before": {
          ...
          inset: 0,
          background:
            "linear-gradient(135deg, rgba(255,255,255,0.22) 0%, rgba(255,255,255,0) 70%)",
          pointerEvents: "none",
          borderRadius: 8,
        },
      }}
    >
      ...
      <Divider sx={{ mb: 2, borderColor: palette.borderSubtle }} />
        ...
        >
          <Select
            ...
            sx={{
              ...
              "&:before, &:after": {
                borderBottomColor: palette.primaryDark + " !important",
              },
              "& .MuiSelect-select": { py: 1 },
              "&:hover": { background: "#f0faff" },
              "&.Mui-focused": { background: "#eef9ff" },
            }}
            MenuProps={{
              PaperProps: {
                sx: {
          ...
          <Button
            ...
            sx={{
              ...
              "&:hover": {
                ...
                boxShadow: "0 10px 26px -6px rgba(0,0,0,0.55)",
              },
  ...
  const fetchTicketStats = async () => {
    ...
    try {
      ...
      const filterDataFinalizada = stats.filter(
        (item) => item.estado === "FINALIZADO" || item.estado === "NO APLICA"
      );

      // Combinar todos los elementos de filterDataFinalizada en un solo diccionario
      const combinedStatsFinalizado = filterDataFinalizada.reduce(
        (acc, item) => {
          return {
            ...acc,
            Q: (acc.Q || 0) + item.Q,
  ...
  const pieChartData = () => {
    ...
    const totalTicketsExcludingFinalized = stats.reduce(
      (acc, item) => acc + item.Q,
      0
    );
    const pieData = stats.map((item, index) => ({
      ...
      value: item.Q,
      label: `${item.estado} (${(
        (item.Q / totalTicketsExcludingFinalized) *
        100
      ).toFixed(1)}%)`,
      estado: item.estado,
    }));
      ...
      >
        ...
        >
          <PieChart
            ...
            colors={pieData.map(
              (d) => estadoColors[d.estado] || palette.accent
            )}
            sx={{ cursor: "pointer" }}
          />
        </Box>
      ...
      >
        <ModuleHeader
          title="Gestión de Tickets"
            subtitle="Visualiza, filtra y monitorea el estado de los tickets activos."
        />
        {open && (
          <Alert
            onClose={handleClose}
