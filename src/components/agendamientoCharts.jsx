...
import { getDataAgendamientos } from "../api/despachoAPI";
...
function AgendamientoCharts() {
  ...
  const [fecha, setFecha] = useState(new Date().toISOString().slice(0, 10)); // Updated to initialize with the current day

  const fetchChartData = async () => {
    setIsSubmitting(true);
    setIsLoading(true);
    try {
      const response = await getDataAgendamientos(token, fecha);
